"""
Extended Setup script to create comprehensive SmartScribble PostgreSQL database
...
def create_database_if_not_exists(config):
    ...
    if exists:
        print(f"Database '{config['database']}' already exists. Dropping and recreating...")
        ...
        print(f"✓ Dropped existing database '{config['database']}'")
...
def setup_extended_database():
    """
    Setup the comprehensive SmartScribble PostgreSQL database
    """
    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE orders (
                ...
                order_status VARCHAR(20) DEFAULT 'pending' CHECK (order_status IN ('pending', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded', 'returned')),
        ...
        cursor.execute('''
            CREATE TABLE shipments (
                ...
                shipment_status VARCHAR(20) DEFAULT 'preparing' CHECK (shipment_status IN ('preparing', 'in_transit', 'out_for_delivery', 'delivered', 'failed', 'returned')),
                notes TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        ...
        cursor.execute('''
            CREATE TABLE support_tickets (
                ...
                category VARCHAR(50) NOT NULL CHECK (category IN ('Technical', 'Billing', 'Shipping', 'Product Inquiry', 'Return/Refund', 'Other')),
        ...
        cursor.execute('''
            CREATE TABLE returns (
                ...
                return_type VARCHAR(20) NOT NULL CHECK (return_type IN ('refund', 'exchange', 'warranty')),
                return_status VARCHAR(20) DEFAULT 'requested' CHECK (return_status IN ('requested', 'approved', 'rejected', 'received', 'processing', 'completed')),
                quantity INTEGER NOT NULL DEFAULT 1,
                refund_amount DECIMAL(10, 2),
                return_shipping_label VARCHAR(100),
        ...
        cursor.execute('''
            CREATE TABLE inventory_logs (
                ...
                product_id INTEGER NOT NULL,
                change_type VARCHAR(20) NOT NULL CHECK (change_type IN ('restock', 'sale', 'return', 'damage', 'adjustment')),
        ...
        products_data = [
            ('SmartScribble AI Notebook', 'SSAI-NB-001', 'Hardware', 'Revolutionary AI-powered digital notebook with E-ink display and pressure-sensitive stylus', 299.00, 500, 50, True, '2024-01-01', 12, 0.395, '230mm x 190mm x 5.8mm'),
            ('Replacement Stylus', 'SSAI-STY-001', 'Accessory', 'Pressure-sensitive replacement stylus with 4096 levels of pressure', 29.00, 1000, 100, True, '2024-01-01', 6, 0.015, '140mm length'),
            ('Premium Leather Folio', 'SSAI-FOL-001', 'Accessory', 'Handcrafted leather folio case with document pocket', 79.00, 200, 25, True, '2024-02-01', 12, 0.250, '250mm x 210mm x 15mm'),
            ('Screen Protector (2-pack)', 'SSAI-SP-001', 'Accessory', 'Anti-glare tempered glass screen protector', 19.00, 500, 50, True, '2024-01-01', 0, 0.050, '230mm x 190mm'),
            ('USB-C Charging Cable', 'SSAI-CAB-001', 'Accessory', 'Braided USB-C charging cable 2m', 15.00, 800, 100, True, '2024-01-01', 12, 0.080, '2000mm length'),
            ('Replacement Nibs (10-pack)', 'SSAI-NIB-001', 'Accessory', 'Felt-composite replacement nibs for stylus', 12.00, 600, 75, True, '2024-01-01', 0, 0.010, '5mm diameter'),
            ('ScribbleCare+ Protection Plan', 'SSAI-CARE-001', 'Service', '2-year accidental damage protection', 49.00, 9999, 0, True, '2024-01-01', 24, 0.0, 'Digital Service'),
        ]
        cursor.executemany('''
        ...
        tickets_data = [
            ('TKT-2024-0001', 1, 1, 'Display not turning on', 'Technical', 'high', 'resolved', 'Device won\'t power on after charging overnight', 'Sent replacement unit', 'Agent Smith', '2024-01-20 09:00:00', '2024-01-22 16:30:00', 45, 3270, 5),
            ('TKT-2024-0002', 3, None, 'How to sync with Evernote?', 'Product Inquiry', 'low', 'closed', 'Need help setting up Evernote sync', 'Provided setup guide link', 'Agent Johnson', '2024-03-15 14:20:00', '2024-03-15 15:45:00', 15, 85, 5),
            ('TKT-2024-0003', 7, 6, 'Request refund for defective unit', 'Return/Refund', 'high', 'resolved', 'Display has dead pixels, requesting refund', 'Refund processed, return label sent', 'Agent Davis', '2024-07-23 10:00:00', '2024-07-28 14:00:00', 60, 7140, 4),
            ('TKT-2024-0004', 5, 19, 'Order not received', 'Shipping', 'urgent', 'resolved', 'Order was supposed to arrive 3 days ago', 'Located package, delivered next day', 'Agent Martinez', '2024-05-20 08:30:00', '2024-05-21 17:00:00', 30, 1950, 4),
            ('TKT-2024-0005', 12, None, 'Stylus not pairing', 'Technical', 'medium', 'closed', 'Replacement stylus doesn\'t seem to work', 'Guided through reset process', 'Agent Lee', '2024-12-18 11:00:00', '2024-12-18 12:30:00', 20, 90, 5),
            ('TKT-2025-0006', 9, None, 'Bulk discount inquiry', 'Billing', 'low', 'closed', 'Want to purchase 10 units for office, any discounts?', 'Provided enterprise sales contact', 'Agent Wilson', '2025-01-10 09:45:00', '2025-01-10 10:15:00', 10, 30, 5),
            ('TKT-2025-0007', 18, 36, 'Exchange for new unit', 'Return/Refund', 'medium', 'in_progress', 'Stylus tracking issue, want exchange', 'Processing exchange', 'Agent Brown', '2025-01-13 14:00:00', None, 25, None, None),
            ('TKT-2025-0008', 15, 26, 'When will pre-order ship?', 'Shipping', 'low', 'open', 'Pre-ordered in December, need shipping date', None, 'Agent Taylor', '2025-01-20 16:00:00', None, None, None, None),
        ]

        cursor.executemany('''
            INSERT INTO support_tickets (ticket_number, customer_id, order_id, subject, category, priority, status, description, resolution, assigned_to, opened_date, closed_date, first_response_time_minutes, resolution_time_minutes, satisfaction_rating)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        ''', tickets_data)
        ...
        messages_data = [
            (1, 'customer', 'Sarah Johnson', 'My device won\'t turn on. I charged it all night but nothing happens when I press the power button.', False, '2024-01-20 09:00:00'),
            (1, 'agent', 'Agent Smith', 'Thank you for contacting us. Let\'s try a hard reset. Please hold the power button for 30 seconds.', False, '2024-01-20 09:45:00'),
            (1, 'customer', 'Sarah Johnson', 'I tried that but still nothing. The LED doesn\'t even light up.', False, '2024-01-20 10:15:00'),
            (1, 'agent', 'Agent Smith', 'I apologize for the inconvenience. This appears to be a hardware issue. I\'m approving a replacement unit to be shipped immediately.', False, '2024-01-20 11:00:00'),
            (2, 'customer', 'Emily Rodriguez', 'I just got my SmartScribble and want to sync with Evernote. Where do I find this setting?', False, '2024-03-15 14:20:00'),
            (2, 'agent', 'Agent Johnson', 'Go to Settings > Cloud Sync > Add Service > Select Evernote. You\'ll need to authorize the connection. Here\'s a detailed guide: [link]', False, '2024-03-15 14:35:00'),
            (3, 'customer', 'Amanda Taylor', 'I received the device but the display has several dead pixels in the center. This is unusable. I want a refund.', False, '2024-07-23 10:00:00'),
            (3, 'agent', 'Agent Davis', 'I\'m very sorry to hear that. We stand behind our quality. I\'m processing your refund right away and emailing you a prepaid return label.', False, '2024-07-23 11:00:00'),
        ]
        ...
        reviews_data = [
            (1, 1, 1, 5, 'Life-changing productivity tool!', 'I\'ve been using this for 3 months and it\'s transformed how I take notes. The AI features are incredible.', True, 45, 0, 'approved', '2024-02-01 10:00:00'),
            (1, 3, 10, 5, 'Perfect for artists', 'The pressure sensitivity is amazing. I can sketch just like on paper but with all the digital benefits.', True, 32, 0, 'approved', '2024-04-10 15:30:00'),
            (1, 5, 19, 4, 'Great for students', 'Love it for taking lecture notes. OCR works really well. Only wish battery lasted longer.', True, 28, 0, 'approved', '2024-06-05 09:15:00'),
            (1, 7, 6, 3, 'Good but pricey', 'Works as advertised but $299 is steep. Worth it if you write a lot.', True, 15, 2, 'approved', '2024-08-12 14:00:00'),
            (1, 9, None, 5, 'Best purchase of 2024', 'I run a business and use this for all my meetings and brainstorming. The AI idea expansion is genius.', True, 67, 0, 'approved', '2024-10-20 11:30:00'),
            (2, 1, 2, 4, 'Good replacement', 'Works just like the original. Delivered quickly.', True, 8, 0, 'approved', '2024-03-05 16:45:00'),
            (2, 13, None, 5, 'Essential backup', 'Lost my original stylus, this one works perfectly. Keep an extra on hand!', True, 12, 0, 'approved', '2024-12-15 10:20:00'),
        ]
        ...
        promotions_data = [
            ('STUDENT10', 'Student Discount', 'percentage', 10.0, 0.0, None, 'SSAI-NB-001', '2024-01-01', '2025-12-31', None, 15, True),
            ('BULK5', 'Bulk Purchase Discount', 'percentage', 10.0, 500.0, None, 'ALL', '2024-01-01', '2025-12-31', None, 3, True),
            ('WELCOME15', 'New Customer Welcome', 'percentage', 15.0, 0.0, 50.0, 'ALL', '2024-01-01', '2024-12-31', 500, 234, False),
            ('STYLUS5OFF', 'Stylus Discount', 'fixed_amount', 5.0, 0.0, None, 'SSAI-STY-001', '2024-06-01', '2024-08-31', None, 89, False),
        ]
...
if __name__ == "__main__":
    print("="*80)
    print("SmartScribble Extended PostgreSQL Database Setup")
    print("="*80)
    print()

    success = setup_extended_database()
    ...
    if success:
        ...
        print("\nMake sure to create a .env file with your PostgreSQL credentials:")
        print("  POSTGRES_HOST=localhost")
        print("  POSTGRES_PORT=5432")
        print("  POSTGRES_DATABASE=smartscribble")
